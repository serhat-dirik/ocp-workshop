...
----
{% if PARKSMAP_PY %}
{{GOGS_URL}}/{{GOGS_USER}}/nationalparks-py/tree/{{NATIONALPARKS_VERSION}}
{% else %}
{{GOGS_URL}}/{{GOGS_USER}}/nationalparks
{% endif %}
----
...
On the new page click on the blue "Add Webhook" button on the top right of the form and
choose "Gogs" from the popup menu:

image::ocp-webhook2-gogs.png[Webhook]
...
Boom! From now on, every time you commit new source code to your git
repository, a new build and deploy will occur inside of OpenShift.  Let's try
this out as clicking "Test Delivery" button on the bottom right of the form.

#### Exercise: Using Web Hooks

{% if PARKSMAP_PY %}
Be sure you return to the proper tag in the git repository:

*link:{{GOGS_URL}}/{{GOGS_USER}}/nationalparks-py/tree/{{NATIONALPARKS_VERSION}}[]*
...
Click "Files" at the Gogs page. This is Gogs's repository view. Make sure that the
drop-down menu at the upper left is set for the `{{NATIONALPARKS_VERSION}}` branch. Navigate to the
following path:
...
image::ocp-webhook5-gogs.png[Webhook]
